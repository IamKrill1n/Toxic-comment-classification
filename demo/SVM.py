...
class SVM:
    def __init__(self) -> None:
        self.char_vectorizer = load('model_checkpoint/svm/char_vectorizer.bin')
        self.word_vectorizer = load('model_checkpoint/svm/word_vectorizer.bin')
        self.loaded_models = []
        for i in range(6):
            self.loaded_models.append(load('model_checkpoint/svm/svm_%d.bin' % i))
