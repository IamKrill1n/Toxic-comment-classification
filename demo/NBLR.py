...
from models import NbLrClassifier

class NBLR:
    def __init__(self) -> None:
        self.char_vectorizer = load('model_checkpoint/nb_Lr/char_vectorizer.bin')
        self.word_vectorizer = load('model_checkpoint/nb_Lr/word_vectorizer.bin')
        self.loaded_models = []
        for i in range(6):
            self.loaded_models.append(load('model_checkpoint/nb_Lr/nb_Lr_%d.bin' % i))
